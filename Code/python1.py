...
def two(input):
	word = input.lower()
	count = word.count("bert")
	if count < 2:
		return ""
	word = word.split("bert")
	return word[1]
...
def five(input):
	...
	while count <= len(input)-1:
		if input[count+2] == 'False':
			if input[count] not in nameList:
				nameList.append(input[count])
		count += 4
	return nameList
...
def six(input):
	...
	for count in range(0, len(word)):
		if word[count].lower() == 'c':
			if word[count+1].lower() == "e":
				if word[count+2].lower() == 'i':
					return True
		elif word[count].lower() == 'i' and word[count+1].lower() == 'e' and word[count-1].lower() != 'c' :
			return True
	return False
...
def eight(input):
	numCount = 1
	for loopCount in range(1, int(input)+1):
		numCount *= loopCount
	return numCount
